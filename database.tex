...
\begin{itemize}
	\item 引入：
	\begin{itemize}
		\item 在执行选择操作时，有时候需要对记录排序。如升序查询\lstinline|asc|和降序
		查询\lstinline|desc|。
		\item 数据体量大，分pages存储，无法把全部记录装到主存后进行快排。
		\item 需要使用外部排序从而最小化磁盘读写。
	\end{itemize}
\end{itemize}
...
\newpage
\appendix
\section{附录1：作业二{\footnotesize 苏睿熹22330100}}
...
\begin{enumerate}
	...
	\begin{itemize}
		\item BRANCH：\underline{branch-name} $\|$ branch-city $\|$ assets
		\item CUSTOMER：\underline{customer-id} $\|$ customer-name $\|$ 
		customer-street $\|$ customer-city
		\item EMPLOYEE：\underline{employee-id} $\|$ dependent-name $\|$ 
		employment-length 
		$\|$ start-date $\|$ telephone-number $\|$ employee-name
		\item LOAN：\underline{loan-number} $\|$ amount
		\item ACCOUNT：\underline{account-number} $\|$ balance
	\end{itemize}
\end{enumerate}
